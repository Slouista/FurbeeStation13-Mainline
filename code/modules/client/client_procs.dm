...
/client/New(TopicData)
	...
	if(byond_build > MAX_RECOMMENDED_CLIENT)
		to_chat(src, "<span class='danger'>Your version of byond is newer than the last version explicitly confirmed to work for clients (build [MAX_RECOMMENDED_CLIENT]) This is probably fine.</span>")
		to_chat(src, "<span class='danger'>If you encounter an issue related to this you can go to <a href=\"https://secure.byond.com/download/build\">BYOND's website</a> to download other versions.</span>")
...
/client/proc/set_client_age_from_db(connectiontopic)
	...
	if(CONFIG_GET(flag/panic_bunker) && !holder && !GLOB.deadmins[ckey] && !(ckey in GLOB.bunker_passthrough))
		...
		if(!query_client_in_db_whitelist.Execute())
			...
			return
		if(minutes < living_recs && !query_client_in_db_whitelist.NextRow())
