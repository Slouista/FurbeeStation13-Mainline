...
/mob/living/carbon/human/IsAdvancedToolUser()
	if(HAS_TRAIT(src, TRAIT_MONKEYLIKE))
		if(istype(get_item_by_slot(ITEM_SLOT_HEAD), /obj/item/clothing/head/helmet/monkeytranslator)) //or if they have an uplifter device
			return TRUE
		return FALSE
...
/datum/action/item_action/organ_action/use/monkey_vocal_cords/Trigger()
	if(!IsAvailable())
		return
	if(istype(owner.get_item_by_slot(ITEM_SLOT_HEAD), /obj/item/clothing/head/helmet/monkeytranslator))
